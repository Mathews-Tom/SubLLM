"""Example: SubLLM with Codex provider — single-turn, streaming, multi-turn, batch."""
...
PROVIDER = "codex"
MODEL = f"{PROVIDER}/gpt-5.2"
MODEL_ALT = f"{PROVIDER}/gpt-4.1"
    ...
    for s in statuses:
        if s.provider == PROVIDER:
            ...
            if not s.authenticated:
                console.print("[red]Auth failed. Run `codex login` first.[/]")
                return
    console.print()
